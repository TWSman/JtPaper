...
The yield at low $\jt{}$ stays constant with increasing $\pt{jet}$. At high $\jt{}$ the yield increases and the distributions become wider. Notably, this is due to kinematical limits. Within a fixed cone the maximum $\jt{}$ depends on the track momentum by the relation of $\jt{max} \approx \mathrm{R} \cdot \pt{track}$ and this results an increase of the possible track momenta as jet $\pt{}$ increases. 
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.48\textwidth]{figures/results/JtSignalFinalFitJetPt5.pdf}
