...
The different source of the systematic uncertainty are considered as uncorrelated and the values of each source are summed in quadrature. The resulting uncertainty is 10 \% for the wide component RMS and 13 \% for the narrow component RMS.
...
\caption{Final $\jt{}$ signal distributions with systematic uncertainties shown for different $\pt{jet}$ bins. The distributions get wider with increasing $\pt{jet}$.}
%Fig3_DrawJtSystematics.py in https://github.com/TWSman/JtAnalysis
...
%Fig4and6_DrawRMS.py https://github.com/TWSman/JtAnalysis
\end{center}
\end{figure}

Figure~\ref{fig:jt_highmult} shows comparison of $\jt{}$ distributions between minimum bias and high multiplicity events. Two separate centrality estimators, V0 and ZDC, are used to provide the multiplicity percentiles. Out of these two ZDC is expected to be least sensitive to jet biases in event multiplicity.
...
%Fig5_HighMultiplicityComparison.py in https://github.com/TWSman/JtAnalysis
...
\caption[\textsc{Pythia} $R$ parameters $\jt{}$]{Effect of changing $R$ parameter in jet finding on $\jt{}$ distributions. (a) Comparison of $\jt{}$ signal distributions between different $R$ parameters. (b) Comparison of resulting RMS values.}
%Fig7_DrawRcomparison.py in https://github.com/TWSman/JtAnalysis\label{fig:Rcomparison}
\end{figure}
...
\begin{figure}[htp]
\centering
\includegraphics[width=0.75\textwidth]{figures/results/JetVsLeadingRefConst.pdf}
...
%Fig8_DrawSignalJtRef.py in https://github.com/TWSman/JtAnalysis\label{fig:Rcomparison}
